...
class SelectionTransformer extends StatefulWidget {
  ...
  ///
  /// This transformer will add a newline between each markdown widget selection.
  /// It also handles bullet points and lists.
  SelectionTransformer.markdownWidget({
    super.key,
    required this.child,
  }) : transform = _markdownWidgetSelectionTransform();

  static SelectionTransform _separatedSelectionTransform(String separator) =>
      (selections) => selections.join(separator);

  static SelectionTransform _markdownWidgetSelectionTransform() {
    final numberedListRegExp = RegExp(r'^\d+\.$');
    ...
    return (selections) {
      ...
      for (final selection in selections) {
        if (previous != null) {
          // Trim whitespace for checking conditions.
          final trimmed = previous.trim();

          // Check if the previous line ends with a bullet or matches a numbered list pattern.
          if (trimmed.endsWith('•') || numberedListRegExp.hasMatch(trimmed)) {
            buffer.write(' ');
          } else {
            buffer.write("\n\n");
          }
