...
/// Shared visual states for [VoiceConsoleCard] and [ConsoleMainButton] —
/// used by both live voice mode and standard audio-message recording so
/// they render as the same design.
enum VoiceConsolePhase { idle, listening, thinking, speaking }
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        bloomRect,
        const Radius.circular(outerRadius + 3),
      ),
      Paint()
    ...
    canvas.drawPath(ringPath, Paint()..shader = gradient.createShader(rect));
  }
...
class _OrbPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final bool active =
        ...
        phase == VoiceConsolePhase.speaking;
...
// [child] is the console's content (status/buttons/etc); [overlay] is an
// optional widget pinned to the bottom-right corner inside the card (e.g.
// voice mode's "back to text" pill).
class VoiceConsoleCard extends StatefulWidget {
  const VoiceConsoleCard({
    super.key,
    required this.phase,
    required this.amplitude,
  ...
  @override
  State<VoiceConsoleCard> createState() => _VoiceConsoleCardState();
}

class _VoiceConsoleCardState extends State<VoiceConsoleCard>
  ...
  void initState() {
    ...
    ).animate(CurvedAnimation(parent: _bloomCtrl, curve: Curves.easeInOut));
  }
            ...
            child: ColoredBox(
              ...
              child: Stack(
                children: [
                  ...
                  // Optional overlay — bottom-right corner inside the card
                  if (widget.overlay != null)
                    Positioned(bottom: 10, right: 10, child: widget.overlay!),
                ],
              ),
            ),
          ),
...
class ConsoleMainButton extends StatefulWidget {
  const ConsoleMainButton({
    super.key,
    required this.phase,
    required this.accentColor,
    required this.onTap,
  ...
  @override
  State<ConsoleMainButton> createState() => _ConsoleMainButtonState();
}

class _ConsoleMainButtonState extends State<ConsoleMainButton>
    with TickerProviderStateMixin {
  ...
  @override
  void didUpdateWidget(ConsoleMainButton old) {
  ...
  IconData get _icon => switch (widget.phase) {
    VoiceConsolePhase.idle => Symbols.mic,
    VoiceConsolePhase.listening => Symbols.arrow_upward,
    VoiceConsolePhase.thinking => Symbols.stop,
    VoiceConsolePhase.speaking => Symbols.stop,
  };
  ...
  Widget build(BuildContext context) {
    ...
    final bool showRing =
        widget.phase == VoiceConsolePhase.listening ||
        widget.phase == VoiceConsolePhase.speaking;

    return AnimatedBuilder(
      animation: Listenable.merge([_ringCtrl, _breatheCtrl]),
      builder: (context, _) {
        // Sonar ring: scale 0.9 → 1.9, opacity 1 → 0
        final double ringProgress = Curves.easeOut.transform(_ringCtrl.value);
        final double ringScale = 0.9 + ringProgress * 1.0;
        final double ringOpacity = (1.0 - ringProgress).clamp(0.0, 1.0);
        final double btnScale = widget.phase == VoiceConsolePhase.speaking
            ? _breatheAnim.value
            : 1.0;
...
class ConsoleCancelButton extends StatelessWidget {
  const ConsoleCancelButton({super.key, required this.onTap});
  final VoidCallback onTap;
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Icon(Symbols.close, size: 20, color: fg.withValues(alpha: 0.60)),
      ),
    );
  }
}
